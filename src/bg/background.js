...
for (const web1 of Object.values(webData)) {
  ...
  for (const web2 of Object.values(webData)) {
    ...
    ranksMap[web1.url].push({
      ...
      duration: 10000 + (sameDomain ? Math.random() * 1000 * 60 * 60 : Math.random() * 1000 * 60 * 30) | 0,
    });
  }
}
...
chrome.extension.onMessage.addListener((url, sender, sendResponse) => {
  ...
  while (ranks.length < 3) {
    const web =
      Object.values(webData).filter(web => web.url && isSameDomain(url, web.url)).find(web => ranks.every(rank => rank.web !== web)) ||
      Object.values(webData).find(web => ranks.every(rank => rank.web !== web));
    ranks.push({
      web,
